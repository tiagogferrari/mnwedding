...
export default function GiftSection() {
    ...
    return (
        <section
            id="gifts"
            className="min-h-screen w-full flex items-center justify-center bg-red-200 text-gray-800 snap-start py-12 px-4"
        >
            <div className="max-w-5xl w-full text-center">
                <h2 className="text-3xl md:text-4xl font-serif mb-6">Lista de Presentes</h2>

                <p className="max-w-3xl mx-auto mb-8 text-base italic">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
                    {gifts.map((gift) => (
                        <div key={gift.name} className="border border-gray-300 rounded-lg p-3 flex flex-col justify-between items-center bg-white shadow-md hover:shadow-lg transition-shadow">
                            ...
                            <div className='text-center'>
                                ...
                                <p className="text-gray-600 mt-1 text-xs">{gift.price}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="flex flex-col items-center gap-3">
                    <div className="flex items-center gap-4 bg-white border border-gray-300 rounded-full px-5 py-2 shadow-sm">
                        <span className="text-gray-700 font-mono text-base">{pixKey}</span>
                        <button
                            onClick={handleCopy}
                            className={`px-5 py-1.5 rounded-full text-white text-sm font-semibold transition-all duration-300 ${copied ? 'bg-green-500' : 'bg-gray-800 hover:bg-gray-700'}`}
                        >
